...
def smart_DDP(model):
    ...
    if check_version(torch.__version__, '1.11.0'):
        return DDP(model, device_ids=[LOCAL_RANK], output_device=LOCAL_RANK, static_graph=True, bucket_cap_mb = 500)
    else:
        return DDP(model, device_ids=[LOCAL_RANK], output_device=LOCAL_RANK, bucket_cap_mb = 500)
...
def profile(input, ops, n=10, device=None):
    ...
    for x in input if isinstance(input, list) else [input]:
        ...
        for m in ops if isinstance(ops, list) else [ops]:
            m = m.to(device, non_blocking=True) if hasattr(m, 'to') else m  # device
...
def fuse_conv_and_bn(conv, bn):
    ...
    fusedconv = nn.Conv2d(conv.in_channels,
                          ...
                          groups=conv.groups,
                          bias=True).requires_grad_(False).to(conv.weight.device, non_blocking=True)
...
def smart_optimizer(model, name='Adam', lr=0.001, momentum=0.9, decay=1e-5):
    ...
    if name == 'Adam':
        optimizer = apex.optimizers.NpuFusedAdam(g[2], lr=lr, betas=(momentum, 0.999))  # adjust beta1 to momentum
    elif name == 'AdamW':
        optimizer = apex.optimizers.NpuFusedAdamW(g[2], lr=lr, betas=(momentum, 0.999), weight_decay=0.0, amsgrad=True)
    elif name == 'RMSProp':
        optimizer = apex.optimizers.NpuFusedRMSprop(g[2], lr=lr, momentum=momentum)
    elif name == 'SGD':
