...
FONT = '/home/ma-user/work/yolov9_on_gpu/data/Arial.ttf'  # https://ultralytics.com/assets/Arial.ttf
...
#     参数:
#     - boxes (Tensor): 边界框张量，形状为 [N, 4]，每个框的格式为 [x1, y1, x2, y2]。
#     - scores (Tensor): 置信度分数张量，形状为 [N]。
#     - iou_threshold (float): 用于抑制的 IoU 阈值。

#     返回:
#     - keep (List[int]): 保留的边界框的索引列表。
#     """
#     # 计算每个边界框的面积
#     x1 = boxes[:, 0]
#     y1 = boxes[:, 1]
...
#         order = order[inds + 1]  # inds + 1 是因为第一个元素被保留了
...
def nms_manual(boxes, scores, iou_threshold):
    ...
    while order.numel() > 0:
        ...
        w = (xx2 - xx1 + 1).clamp(min=0.0)
        h = (yy2 - yy1 + 1).clamp(min=0.0)
        inter = w * h
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # i = nms_manual(boxes, scores, iou_thres)
        # i = torch.tensor(i)
        # i = torchvision.ops.nms(boxes, scores, iou_thres)  # NMS
        ...
        output[xi] = x[i]
        if mps:
            output[xi] = output[xi].to(device, non_blocking=True)
...
def apply_classifier(x, model, img, im0):
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            pred_cls2 = model(torch.Tensor(ims).to(d.device, non_blocking=True)).argmax(1)  # classifier prediction
