...
class Conv(nn.Module):
    ...
    # default_act = torch_npu.contrib.module.SiLU()  # default activation
    default_act = nn.SiLU()
    ...
    def _fuse_bn_tensor(self, branch):
        ...
        if isinstance(branch, Conv):
            ...
            if not hasattr(self, 'id_tensor'):
                ...
                self.id_tensor = torch.from_numpy(kernel_value).to(branch.weight.device, non_blocking=True)
...
class BottleneckCSP(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        # self.act = torch_npu.contrib.module.SiLU()
        self.act = nn.SiLU()
...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='yolo.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=True):
        ...
        if pt:  # PyTorch
            ...
            for i in range(model.num_bindings):
                ...
                im = torch.from_numpy(np.empty(shape, dtype=dtype)).to(device, non_blocking=True)
    ...
    def from_numpy(self, x):
        return torch.from_numpy(x).to(self.device, non_blocking=True) if isinstance(x, np.ndarray) else x
        ...
        with dt[0]:
            ...
            if isinstance(ims, torch.Tensor):  # torch
                with amp.autocast(autocast):
                    return self.model(ims.to(p.device, non_blocking=True).type_as(p), augment=augment)  # inference
            ...
            x = torch.from_numpy(x).to(p.device, non_blocking=True).type_as(p) / 255  # uint8 to fp16/32
...
class Classify(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.linear = NpuLinear(c_, c2)  # to x(b,c2)
