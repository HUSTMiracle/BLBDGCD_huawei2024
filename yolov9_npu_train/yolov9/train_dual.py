...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        model = Model(cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device,non_blocking=True)  # create
        ...
        model = Model(cfg, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device,non_blocking=True)  # create
    amp1 = check_amp(model)  # check AMP
    ...
    if RANK == -1 and batch_size == -1:  # single-GPU only, estimate best batch size
        batch_size = check_train_batch_size(model, imgsz, amp1)
    ...
    model.class_weights = labels_to_class_weights(dataset.labels, nc).to(device,non_blocking=True) * nc  # attach class weights
    model.names = names
    ...
    scaler = amp.GradScaler(init_scale=2)
    ...
    #             record_shapes=True,    # 算子的InputShapes和InputTypes，Bool类
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Forward
            with torch.cuda.amp.autocast(amp1):
